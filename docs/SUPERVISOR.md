...
| `pnpm test:framework`                                                   | The framework's own self-tests. Mandatory after any change to `scripts/`, hooks, or configs — `pnpm verify` does NOT cover these (see `FRAMEWORK.md`). Framework-path commits now run it automatically in pre-commit (lefthook `3_framework`). |
| `pnpm mutation`                                                         | The anti-vacuous-test check (slow; CI runs it). Run locally when reviewing a big test contribution. (PRs run it only when mutation-relevant paths change.)                                                                                     |
